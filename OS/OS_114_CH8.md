...
- Page Table 都會在 Main memory 裡面
- 實作當中，會有一組
	- Page-table base register (PTBR) 指向 Page Table
	- Page-table length register (PTLR) 是 Page Table 的長度
- 在 CPU 訪問記憶體時，總共會有兩次，一次是訪問 Page Table，一次是訪問記憶體
- 因為訪問記憶體會有延遲，所以有了硬體的解決方案，稱為 associative memory 或是 translation look-aside buffers (TLBs)
#### Associative Memory
- Parallel search 並列搜尋
	![[Pasted image 20251208130126.png|500]]
![[Pasted image 20251208130215.png|500]]
- 流程是先用硬體提供的 TLB 找，沒在 TLB 找到的，再去訪問 Page Table 找
#### Shared Pages
- Shared code
	- read-only code 又稱 reentrant code，唯讀程式碼無法被修改
	- 會在 physical memory 上
	- 可以透過 Shared Page 分享同一份 code 的副本
- Private code and data
	- 每個 Process 都有屬於自己的資料以及變數
	- 而私有的 Page 可以映射到 Process 的 Logical Address Space 的任何位置
![[Pasted image 20251208132102.png|500]]\
### Structure of the Page Table
- 原先設計問題
	- 當應用程式越來越大，則就需要**更大的 logical address**，**Page Table 就要也變得更大**
	- 且 Page Table 必須是**連續的記憶體**，所以會佔下**龐大的資源**
### Hierarchical Page Tables
![[Pasted image 20251208133557.png|500]]
- 用分層的方式，減少記憶體一次佔的大小，不需要一個連續的記憶體空間
- 但又多佔了一個表的記憶體，並且要再多一次訪問記憶體
![[Pasted image 20251208134413.png|500]]
- 若是表在更大一點，甚至會反其道而行，Outer table 還是會很大
- 但如果再多階層，則會造成更多次的訪問記憶體
### Hashed Page Tables
- 常用於 32 bit 以上的 address spaces
- 用 Hash function 的方法映射
- 可以用 linked list 解決衝突問題
![[Pasted image 20251208151242.png|500]]
### Inverted Page Table
- 傳統：用 Virtual page 查 Physical Frame
- Inverted：用 Frame 紀錄哪個 process 的 page (將順序反過來)

- 只有一個 shared table，無論有多少個 process
- 所有 process 共用同一個 table，所以在查詢的時候會搜尋所有的 entry
- 有多少 frames 就有多少的 entry，取決於實體記憶體有多少
- 減少記憶體用量，但查尋會很慢

- 可以用 Hash table 優化查詢時間
![[Pasted image 20251208152629.png|500]]
- 利用 pid 跟 p 找尋 i
## Segmentation
- **User view** of memory
	- 當你寫程式時，你腦袋裡想的不是「我要把資料存在第 0x0040 號位址」，也不是「我要把程式切成 4KB 的小塊」。
	- 你腦袋裡想的是結構化的東西：**「這是我的主程式 (Main)」、「這是一個函數 (Function)」、「這是一個陣列」、「這是我的堆疊 (Stack)」**。
	- Segmentation 就是順應這種思維，把記憶體依照這些**邏輯單位**來切割，而不是硬生生切成固定大小。
- 以使用者觀點，看的東西，像是 function、object、method、stack 等，讓人比較有可讀性
- one segmentation = one function、one stack、one main program
- 不同大小的空間配置
![[Pasted image 20251208154256.png|500]]
### Architecture
- Logical address 
...
- Segment-table length register (STLR)
	- 紀錄有多少個 segment
	- 每次都會檢查查詢的記憶體序列，一定要小於 STLR，`segment number s is legal if s < STLR`
![[Pasted image 20251208155304.png|500]]
- offset < limit，segment 的大小不能超過 limit
![[Pasted image 20251208155729.png|500]]
## Effective Access Time
- a.k.a EAT
- 使用 TLB 技術計算平均 memory 訪問時間，看命中率的影響
- $EAT=(1+\varepsilon)\alpha+(2+\varepsilon)(1-\alpha)=2+\varepsilon-\alpha$
	- $\varepsilon$ (epsilon)：TLB 搜尋時間 (Associative Lookup)。這通常很快（例如 20ns）。
	- $t$ (Memory cycle)：存取一次主記憶體的時間。範例中設為 1 microsecond (或是下面例子的 100ns)。
	- $\alpha$ (alpha)：Hit Ratio (命中率)。代表在 TLB 就找到頁號的機率。
	- $1 - \alpha$：Miss Ratio (未命中率)。代表必須去查分頁表（Page Table）的機率。
- $(1 + \varepsilon)\alpha$
	- CPU 要找的地址在 TLB 裡找到了
	- (TLB 查詢時間 + 抓資料時間)\*機率
- $(2 + \varepsilon)(1 - \alpha)$
	- TLB 沒找到，必須走傳統路徑
	- (TLB 查詢時間 + 訪問 Page Table + 抓資料時間)\*(1 - 機率)
