...
# 安全軟體 講稿
### **Speech Script**
Good afternoon, everyone. We are Group 9, and today we’re here to share our presentation with you. Our focus is on analyzing the security of an open-source software called **Labelme**, and we’ll be explaining each step of the SSDLC process in detail.

To begin, let me introduce what Labelme is. Labelme is an image annotation tool that allows users to label and categorize images. Unlike basic tools that only support rectangle annotations, Labelme also supports polygon shapes, enabling more precise labeling. This makes it particularly useful for creating high-accuracy datasets for training. Additionally, Labelme is capable of working with video frames, allowing users to annotate and classify each frame individually. If this tool interests you, its source code is freely available on GitHub.

For this presentation, we chose Labelme as the target of our SSDLC analysis. By applying the six phases of SSDLC, we evaluated the software using eight different testing principles and tools. My teammates will now explain each phase and the tools we used in more detail.

---

After the detailed explanations from my teammates, you should now have a clear understanding of the tools and methods we used. I’ll now summarize what we’ve accomplished with Labelme in terms of security applications.

During the **Requirement Phase**, we applied the OWASP ASVS framework, which outlines 14 security requirements. These help developers identify risks and ensure their applications meet security standards. In our analysis, we focused on several key risks, providing explanations and solutions for each.

In the **Design Phase**, we utilized the Microsoft Threat Modeling Tool to visualize system architecture and detect potential vulnerabilities. This tool generates a list of risks along with recommendations, allowing us to address issues during the design stage.

When it came to the **Implementation Phase**, we employed three key tools to enhance code security. First, SonarQube, a SAST tool, helped us review code, detect vulnerabilities, and highlight areas for improvement. Second, the OWASP Secure Coding Practices provided us with a reliable guideline for ensuring our code met security standards. Third, Safety was used to scan third-party dependencies for vulnerabilities, assess risks, and provide developers with actionable insights to fix potential threats early.

In the **Testing Phase**, we used Mergebase to perform a detailed security scan focused on dependency management. This tool helped identify vulnerabilities, most of which were linked to third-party libraries, and provided us with comprehensive reports for mitigation.

Finally, during the **Deployment and Maintenance Phases**, we ensured the software’s stability and security by using PyInstaller to package the application and deploying it in a container environment. For penetration testing, we relied on Kube-hunter to identify and address risks in the deployed system.

---

In conclusion, this presentation outlined our comprehensive analysis of Labelme’s security practices using SSDLC. Thank you all for your attention, and we’re happy to address any questions or hear your feedback.
